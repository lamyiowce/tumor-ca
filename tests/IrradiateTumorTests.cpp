...
TEST_CASE("Irradiate - basic test") {
    ...
    for (int i = 1; i <= N_TEST_STEPS; i++) {
        SECTION("Iteration " + std::to_string(i)) {
            sprintf(filepath, "../tests/resources/matlab_results/per-function/%d/out-vnw-tr1-st%d-1-Diffusion.json", i, i);
            auto ca1 = Automaton::loadFromFile(filepath, &mre);
            sprintf(filepath, "../tests/resources/matlab_results/per-function/%d/out-vnw-tr1-st%d-2-IrradiateTumor.json", i, i);
